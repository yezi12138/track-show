...
`快应用`源码放在src/scripts/libs/track，`小程序`源码放在plugins/track， 需要在app页面引用，并且初始化一些环境配置
...
可以在环境配置`registerEnv`方法中修改`lifecyclesEnum`或者`appcyclesEnum`
...
注意！要使用插件，必须先配置环境（为了适配多个开发环境），调用`Track.registerEnv(config)`
...
详细配置可以参考各个项目中`~/track/index.js`文件

## 初始化插件

必须先调用`registerEnv`注册环境，然后调用`registerUtraceRule`注册utrace。如果没有注册utrace，则埋点的数据都不会上报，规定必须存在utrace。
...
`additionData`:额外的数据，可以指定环境，或者一些版本号之类的。处于初始化设置外，还可以通过`setAdditionData`方法设置。



## 页面监听:api

通过配置`pageTracks`，实现页面的监听
...
组件其实和页面的配置大致差不多，但是`componentTracks`的key值一定要对应组件的名称：

- 快应用中，组件的名称就是**<import name="create-window" src="～/window/index.ux"></import>**中的**name**,也就是**create-window**
...
### 监听元素的曝光
目前仅支持`小程序`
配置页面的`observer`,目前仅支持单个类，也就是说如果在对象observer里面，只能存在一个字段类（后续开发再优化）
`dataset`：就是在元素上绑定的自定义值,比如<div data-id="123"></div>,拿到的值就是{ id: '123' }
...
```
修改曝光上传的接口,修改registerEnv中的`bindObserver`函数
```
...
配置`registerEnv`的api`lifecycles`
